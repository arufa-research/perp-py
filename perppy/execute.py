...
class ExecuteConnector:
    ...
    def _approve_layer1_bridge( # bridge from layer 1 to layer 2
        ...
        layer1_bridge_addr = MetaData().get_layer1_contract('RootBridge')
        ...
        approve_transaction = layer1_usdc_contract.functions.approve(layer1_bridge_addr, 2**256-1).buildTransaction({
            'nonce': nonce,
            'gas': 1000000,
    ...
    def deposit_to_layer2(
        ...
        receipt = self._approve_layer1_bridge()
    ...
    def open_position(
        ...
        transaction = clearing_house_contract.functions.openPosition(amm_addr, side, {'d': quote_asset_amount}, {'d':leverage}, {'d':base_asset_amount_limit}).buildTransaction({
            'nonce':nonce,
            'gas': 1000000,
